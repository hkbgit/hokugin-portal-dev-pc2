...
$('#kyaraForm').submit(function(event) {
  console.log("kyarFormvalidatevalidatevalidatevalidate");
  if (isValidated) {
    return;
  }
  ...
  $.ajax({
    url : '/back/kyaraRegister/validate',
  ...
  .done(function(data) {
    console.log("/back/kyaraRegister/validate  成功");
    removeErrorInfo();
    if (!data) {
      console.log("/back/kyaraRegister/validate  成功1"); 
      // バリデーション異常なしの場合
      isValidated = true;
      $('#kyaraForm').submit();
      ...
      if (errorInfo.name) {
        ...
        $('#errorName').text(errorInfo.name.msg);
      }
