 ...
 router.post('/', upload, async function(req, res, next) {
  ...
  if (req.body.kyaraId) {
    ...
    if (req.files.image || req.files.csv ) {
      ...
      // バナー画像は更新しない場合
      cloudantKyaraModel.updateItemCallback(req, res, session, updateItemCallback);
    }
  } else {
    // バナー新規追加処理
    if (!req.files.image) {
 ...
 const checkUploadedFile4 = function(file) {
  ...
  try {
    ...
    // for (const token of constants.FILE_DIMENSIONS_LIST) {
      if (width == constants.FILE_DIMENSION_KYARA.WIDTH && height == constants.FILE_DIMENSION_KYARA.HEIGHT  ) {
...
const validateInputValue2 = function(req) {
  ...
  formValues.name = req.body.name ? req.body.name : EMPTY_VALUE;
 ...
 const saveImage = async function(req, res, session, file, isUpdateBanner = false) {
  ...
  try {
    ...
    try {
      ...
      if (isUpdateBanner) {
        // 既存のimageレコードを更新する
        let kyaraInfo = await cloudantKyaraModel.getItemById(req.body.kyaraId);
        if (kyaraInfo.length !== 1) {
          throw Error('faild to get kyaraInfo');
        }
        let originImagePath = kyaraInfo[0].image_path;
      ...
      try {
        await sessionModel.addErrorMsg(session,constants.MESSAGE.FAILED_TO_REGISTER_KYARA);
    ...
    if (isUpdateBanner) {
      cloudantKyaraModel.updateItemCallback(req, res, session, updateItemCallback, dest);
    } else {
      cloudantKyaraModel.createItem(req, res, session, createItemCallback, dest);
    }
    ...
    if (isUpdateBanner) {
      ...
      res.redirect(constants.ROUTE.REGISTER);
    }
 ...
 */
 const createItemCallback = async(err, req, res, result) => {
  if (err) {
    ...
    try {
      await sessionModel.sessionAddErrorMsg(req,constants.MESSAGE.FAILED_TO_REGISTER_KYARA);
      res.redirect(constants.ROUTE.REGISTER_KYARA);
    } catch (error) {
      logger.error('save session errorMsg error: '+error);
      sessionManager.destroy(req);//这里只是将系统的session删除了，并没有删除数据库的
      msg = constants.CLOUDANT.RES_MESSAGE.OTHER_ERROR;
